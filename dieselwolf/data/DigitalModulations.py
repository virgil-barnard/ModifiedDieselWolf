...
from .FilterUtils import RRC_filt_RealSpace
...
class DigitalModulationDataset(Dataset):
    ...
    def __init__(
        ...
        self.rrc_n_taps = numba.int32(
            self.rrc_n_taps - ((self.rrc_n_taps - 1) % 2)
        )  # odd
        ...
        for c in self.classes:  # modulations loop
            for _ in range(self.num_examples):  # examples loop
                ...
                oversamp = self.rng.integers(
                    self.min_samp, self.max_samp, endpoint=True
                )
                num_symbols = self.num_samples / oversamp
                num_symbols = int(
                    np.ceil(num_symbols)
                )  # one extra partial symbol at end

                # getting RRC alpha
                alpha = self.rng.uniform(
                    low=self.min_RRC_alpha, high=self.max_RRC_alpha
                )

                if self.return_message:
                    # makes random symbol strings, (see GetModulation in ModulationTypes.py)
                    if self.seed is not None:
                        (symbols_I, symbols_Q), symbols = GetModulation(
                            c, num_symbols, self.rng
                        )
                    else:
                    ...
                    # makes random symbol strings, (see GetModulation in ModulationTypes.py)
                    if self.seed is not None:
                        (symbols_I, symbols_Q), _ = GetModulation(
                            c, num_symbols, self.rng
                        )
                    else:
                ...
                digital_I = RRC_filt_RealSpace(
                    digital_I, self.rrc_n_taps, alpha, self.data_rate, 1 / float(dt)
                )
                digital_Q = RRC_filt_RealSpace(
                    digital_Q, self.rrc_n_taps, alpha, self.data_rate, 1 / float(dt)
                )
                ...
                if diff_index != 0:
                    ...
                    digital_I = digital_I[
                        random_index : random_index + self.num_samples
                    ]
                    digital_Q = digital_Q[
                        random_index : random_index + self.num_samples
                    ]
                ...
                self.data.append(
                    [
                        ...
                        oversamp,
                        dt,
                        [digital_I.astype("float32"), digital_Q.astype("float32")],
                    ]
                )
    ...
    def __getitem__(self, idx):
        ...
        examp = self.data[idx]
        data = torch.from_numpy(np.stack(examp[-1]))
        label = examp[0]
        meta = {
            "num_symbols": examp[1],
            "RRC_alpha": examp[2],
