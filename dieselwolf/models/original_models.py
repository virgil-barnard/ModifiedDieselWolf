...
# from reformer_pytorch import Reformer # ERROR: Not in our environment
...
class ConvUpsampleBlock(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size, padding, bias, activation=True
    ):
...
# ERROR: Reformer not in our environment, must replace with our custom complext_transformer.py
...
class BasicResidualStack(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, downsample=None):
        ...
        self.basic_unit_1 = BasicResidualUnit(
            in_channels=out_channels, out_channels=out_channels
        )
        self.basic_unit_2 = BasicResidualUnit(
            in_channels=out_channels, out_channels=out_channels
        )
...
class Basic1DModel(nn.Module):
    ...
    def __init__(self, in_size, in_channels, block_sizes, num_classes):
        ...
        self.basic_stack_1 = BasicResidualStack(
            in_channels,
            block_sizes[0],
        )
        num_features /= 2

        self.basic_stack_2 = BasicResidualStack(
            block_sizes[0],
            block_sizes[1],
        )
        num_features /= 2

        self.basic_stack_3 = BasicResidualStack(
            block_sizes[1],
            block_sizes[2],
        )
        num_features /= 2
        ...
        if len(self.block_sizes) >= 4:
            self.basic_stack_4 = BasicResidualStack(
                block_sizes[2],
                block_sizes[3],
            )
            num_features /= 2
        ...
        if len(self.block_sizes) >= 5:
            self.basic_stack_5 = BasicResidualStack(
                block_sizes[3],
                block_sizes[4],
            )
            num_features /= 2
        ...
        if len(self.block_sizes) >= 6:
            self.basic_stack_6 = BasicResidualStack(
                block_sizes[4],
                block_sizes[5],
            )
            num_features /= 2
        ...
        self.fc_1 = nn.Linear(
            in_features=int(block_sizes[-1] * num_features),
            out_features=128,
        )
        self.selu_1 = nn.SELU(inplace=True)
        self.a_dropout_1 = nn.AlphaDropout(0.1)

        self.fc_2 = nn.Linear(128, 128)
        self.selu_2 = nn.SELU(inplace=True)
        self.a_dropout_2 = nn.AlphaDropout(0.1)
    ...
    def forward(self, input_tensor):
        ...
        if len(self.block_sizes) >= 4:
            output_tensor = self.basic_stack_4(output_tensor)
        if len(self.block_sizes) >= 5:
            output_tensor = self.basic_stack_5(output_tensor)
        if len(self.block_sizes) >= 6:
            output_tensor = self.basic_stack_6(output_tensor)
